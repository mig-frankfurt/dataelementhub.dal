...
(
    ...
    CONSTRAINT source_id_fkey FOREIGN KEY (source_id) REFERENCES source (id) ON DELETE CASCADE,
    CONSTRAINT user_id_fkey FOREIGN KEY (user_id) REFERENCES dehub_user (id) ON DELETE CASCADE
);
...
(
    ...
    left_source  integer       NOT NULL,
    right_urn    TEXT          NOT NULL,
    right_source integer       NOT NULL,
    relation     relation_type NOT NULL,
    created_by   INTEGER       NOT NULL,
    created_at   timestamp     NOT NULL DEFAULT current_timestamp,
    CONSTRAINT element_relation_pkey PRIMARY KEY (left_urn, left_source,
                                                  right_urn, right_source),
    CONSTRAINT left_source_id_fkey FOREIGN KEY (left_source) REFERENCES source (id) ON DELETE CASCADE,
    CONSTRAINT right_source_id_fkey FOREIGN KEY (right_source) REFERENCES source (id) ON DELETE CASCADE
);

CREATE TABLE value_domain_permissible_value
(
    value_domain_scoped_identifier_id      INTEGER not null,
    permissible_value_scoped_identifier_id INTEGER not null
);
...
ALTER TABLE "element"
    ADD CONSTRAINT element_created_by_fkey FOREIGN KEY (created_by) REFERENCES dehub_user (id);
ALTER TABLE "element"
...
ALTER TABLE scoped_identifier
    ADD CONSTRAINT scoped_identifier_created_by_fkey FOREIGN KEY (created_by) REFERENCES dehub_user (id);
ALTER TABLE scoped_identifier
    ADD CONSTRAINT scoped_identifier_element_id_fkey FOREIGN KEY (element_id) REFERENCES element (id);
ALTER TABLE scoped_identifier
    ADD CONSTRAINT scoped_identifier_namespace_id2_fkey FOREIGN KEY (namespace_id) REFERENCES element (id) ON DELETE CASCADE;
...
ALTER TABLE user_namespace_grants
    ADD CONSTRAINT user_namespace_grants_user_fkey FOREIGN KEY (user_id) REFERENCES dehub_user (id) ON DELETE CASCADE;

ALTER TABLE value_domain_permissible_value
    ADD FOREIGN KEY (value_domain_scoped_identifier_id) references scoped_identifier (id) ON DELETE CASCADE;
ALTER TABLE value_domain_permissible_value
    ADD FOREIGN KEY (permissible_value_scoped_identifier_id) references scoped_identifier (id) ON DELETE CASCADE;